...
| `labelwatch.ingest.cursor_continuity` | Every active, accessible source has a durable cursor observed in the last 15m. A successful unchanged/empty poll renews observation but not advancement. Inactive historical cursors remain retained and visible separately. | The active source set is unavailable/empty (`UNKNOWN`), an active source lacks a durable cursor (`DEGRADED`), or an active durable cursor lacks a current observation (`UNKNOWN`/`STALE`). Inactive retention cannot satisfy or stale active continuity. |
...
| `labelwatch.persistence.sqlite_continuity` | Bounded reads can resolve the schema version and critical tables/index, and a `BEGIN IMMEDIATE` transaction can be acquired and rolled back. | The file is absent, a required object/read is unavailable, or write intent cannot be acquired. This deliberately does **not** run `quick_check`/`integrity_check` and does not establish database integrity. |
...
The corresponding semantic question IDs are the versioned
`labelwatch.question.*` values in the manifest. Cursor continuity uses
`labelwatch.question.ingest_cursor_continuity/v2`; v1 did not distinguish the
active acquisition set from inactive retained cursor history.
...
Polling cursors survive restart as historical continuity facts. A successful
poll using an unchanged durable cursor renews `observed_at` but never
`advanced_at`. Inactive cursor history is retained without being treated as
current acquisition evidence. A pre-instrumentation active cursor is `UNKNOWN`.
