...
| `labelwatch.ingest.cursor_continuity` | Active, accessible sources with durable cursors were observed in the last 15m. A successful unchanged/empty poll renews observation but not advancement. Inactive historical cursors remain retained and visible separately. | The active source set is unavailable/empty, no active source has a durable cursor, or an active durable cursor lacks a current observation. Inactive retention cannot satisfy or stale active continuity. |
